...
const chunkFill = (chunkNumber, chunkSize, arr) => {
    ...
    while (idx < len) {
        ...
        const sumNum = sum(chunk)
        const ampVal = Math.round((sumNum / chunk.length + 1) + 3)
...
export default (waveData) => {
    ...
    const itemsInChunk = Math.ceil(waveData.length / displayableChunks)
    console.log('itemsInChunk', itemsInChunk)
